...
router.post('/:id/execute', authenticate, checkPlanAccess('retargeting'), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
...
router.get('/analytics', authenticate, checkPlanAccess('retargeting'), async (req: AuthRequest, res) => {
  try {
