    ...
    for (const field of fields) {
      if (!field.type || !field.label) {
        return res.status(400).json({ 
          error: 'Each field must have type and label' 
        });
      }
    }
...
// Public form submission endpoint (for WhatsApp webview)
...
router.get('/:id/webview-url', authenticate, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
